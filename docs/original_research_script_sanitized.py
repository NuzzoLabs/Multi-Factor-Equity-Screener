...
class AlphaVantageClient:
    ...
    def export_stock_factors_to_excel(self, normal_df, macro_df, output_file="C:/Users/nnuzz_rpjwflj/OneDrive/Documents/Finances/Projects/2026 Q1 Portfolio Analysis and Rebalance/Stock_Factors_ALL_Analysis_5yr_3.xlsx"):
        """ 
...
# -------------------------------------------------------------
# MAIN EXECUTION BODY – ALPHA VANTAGE DATA COLLECTION & FACTOR MODEL
# -------------------------------------------------------------

# Step 1: Select which Alpha Vantage API key to use
# 0 = Free key, 1 = Premium key
key_selection = input("Select API Key (0=Free,1=Premium): ")
api_key = ["YOUR_FREE_API_KEY", "YOUR_PREMIUM_API_KEY"][int(key_selection)] # Premium API subscription canceled as of 2/22/26. Re-subscribe when performing portfolio rebalance

# Initialize AlphaVantageClient with the selected API key
client = AlphaVantageClient(api_key=api_key)
print("API key loaded")

# -------------------------------------------------------------
# Step 2: Define stock tickers to analyze
# -------------------------------------------------------------
#tickers = ['AAPL', 'MSFT', 'IBM', 'JOBY']


tickers = ['JOBY', 'COP', 'MAIN', 'SMCI', 'CORT',
           'BAC', 'TGT', 'AMZN', 'XOM', 'HIMS', 'OKLO', 'PG', 'ARCC', 'JNJ', 'CVX']


# Collect all relevant financial data from Alpha Vantage
Stock_Data = client.Collect_Data(tickers)

# Extract structured financial statements from the raw API data
BalSheets = [Stock_Data['Balance_Sheet'][i]['annualReports'] for i,val in enumerate(Stock_Data['Balance_Sheet'])]
IncomeStatements = [Stock_Data['Income_Statement'][i]['annualReports'] for i,val in enumerate(Stock_Data['Income_Statement'])]
CashFlow = [Stock_Data['Cash_Flow'][i]['annualReports'] for i,val in enumerate(Stock_Data['Cash_Flow'])]
Overview = [Stock_Data['Info'][i] for i,val in enumerate(Stock_Data['Info'])]
Insiders = [Stock_Data['Insider_Activity'][i] for i,val in enumerate(Stock_Data['Insider_Activity'])]
Shares = [Stock_Data['Shares_Outstanding'][i] for i,val in enumerate(Stock_Data['Shares_Outstanding'])]

# -------------------------------------------------------------
# Step 3: Compute Stock Sizes
# - Used to iterate through reports for each stock
# -------------------------------------------------------------
Stock_Size = []
for i in range(len(tickers)):
    stock_size = len(BalSheets[i])  # number of annual reports available
    Stock_Size.append(stock_size)

# -------------------------------------------------------------
# Step 4: Compute key intermediate metrics
# These are used as inputs for factor calculations
# -------------------------------------------------------------

# Compute effective tax rates for each stock
TaxRate = client.compute_TaxRate(Stock_Size, IncomeStatements)

# Compute EBIT (Earnings Before Interest and Taxes)
EBIT = client.compute_EBIT(Stock_Size, IncomeStatements)

# Compute NOPAT (Net Operating Profit After Taxes)
NOPAT = client.compute_NOPAT(EBIT, TaxRate)

# Compute Invested Capital for ROIC and capital efficiency calculations
IC = client.compute_InvestedCapital(Stock_Size, BalSheets)

# Compute Depreciation & Amortization from Income Statements
DandA = client.compute_DepreciationandAmortization(Stock_Size, IncomeStatements)

# Compute changes in Net Working Capital
Del_NWC = client.compute_Del_NWC(Stock_Size, BalSheets)

# Compute net capital expenditures from Cash Flow statements
NetCapEx = client.compute_NetCapEx(Stock_Size, CashFlow)

# Compute Free Cash Flow
FCF = client.compute_FCF(NOPAT, DandA, Del_NWC, NetCapEx)

# Compute total Revenue
Revenue = client.compute_Revenue(Stock_Size, IncomeStatements)

# Compute Return on Invested Capital (ROIC)
ROIC = client.compute_ROIC(Stock_Size, BalSheets, NOPAT, IC)

# -------------------------------------------------------------
# Step 5: Compute factor scores
# Each function returns standardized 0-100 scores per stock
# -------------------------------------------------------------
Liquidity_Factor = client.compute_liquidity_factor(BalSheets, IncomeStatements)
Growth_Factor = client.compute_growth_factor(tickers, BalSheets, CashFlow, IncomeStatements)
CapEff = client.compute_capital_efficiency_factor(NOPAT, IC, FCF, Revenue, ROIC)
Coverage = client.compute_coverage_factor(Stock_Size, Overview, BalSheets, IncomeStatements, FCF, EBIT)
Value = client.compute_value_factor(Stock_Size, Overview, BalSheets, EBIT)
Risk = client.compute_risk_factor(Stock_Size, Insiders, Overview, BalSheets, Revenue)

# -------------------------------------------------------------
# Step 6: Aggregate all factor scores into a single DataFrame
# -------------------------------------------------------------
Factors = client.Factor_Model(Liquidity_Factor, Growth_Factor, CapEff, Coverage, Value, Risk, tickers)
print(f'Factors are \n{Factors}')  # Display the final factor matrix

# -------------------------------------------------------------
# Step 7: Visualize factors using a spider/radar chart
# -------------------------------------------------------------
#client.Spider_Visualize(Factors)

# -------------------------------------------------------------
# Step 8: Export factor scores to Excel for record-keeping or analysis
# -------------------------------------------------------------
macro_weights, factors_adj = client.compute_Macro_Weight(Factors)

print(f'Macroeconomic weightings are {macro_weights} and adjusted factors are {factors_adj}')

client.export_stock_factors_to_excel(Factors, factors_adj)

# -------------------------------------------------------------
# Step 9: Notes for future work
# -------------------------------------------------------------
'''
Class 2: Will handle backtesting of current or new methods.
Class 3: Will evaluate existing portfolios for risk, overweights, and sector/industry concentration.
'''
